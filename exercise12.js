...
// dibelinya). Jika stock barang kurang dari jumlah yang ingin dibeli oleh pembeli maka pembeli batal untuk membeli barang 
// tersebut.
  ...
  if (shoppers.length>0) {  
    for (var i = 0; i < listBarang.length; i++) {
      ...
      for (var j = 0 ; j < shoppers.length ; j++) {
        if (shoppers[j].product===temp.product&&temp.leftOver>=shoppers[j].amount) {
          temp.shoppers.push(shoppers[j].name)
          temp.leftOver = temp.leftOver - shoppers[j].amount
          temp.totalProfit = temp.totalProfit + (listBarang[i][1]*shoppers[j].amount)
        }
      }
      result.push(temp)
    }

...
// Level kedalaman array akan di-asumsikan selalu 3, [ [ [ ] ] ], jika yang diberikan adalah array kosong maka kembalikan 
// nilai 'No number'
  ...
  for (var i = 0 ; i < arr.length ; i++) {
    for (var j = 0 ; j < arr[i].length ; j++) {
      for (var k = 0 ; k < arr[i][j].length ; k++) {
        if (!result) var result = arr[i][j][k]
        else result += arr[i][j][k]
      }
    }
  }
  if (!result||typeof result === NaN) return 'No Number'
  else return result
}
